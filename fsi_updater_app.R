...
# Определяем UI
ui <- fluidPage(
  titlePanel("FSI updater"),
  
  tabsetPanel(
    tabPanel("MOEX Stock Prices",
             actionButton("update", "Get Prices"),
             textInput("priceTextOutput", label = NULL, value = "", width = "400px"),
             tableOutput("priceTable")
    ),
    
    tabPanel("FSI Components",
             actionButton("get_data", "Get data"),
             fluidRow(
               column(6, textInput("ifx_ofzzc", label = "Previous date bond yields:", value = "", width = "95%")),
               column(6, textInput("miacr_ruonia", label = "Previous date money market rates:", value = "", width = "95%"))
             ),
             fluidRow(
...
# Определяем серверную логику
server <- function(input, output, session) {
  
  # Реактивные данные для первой вкладки
